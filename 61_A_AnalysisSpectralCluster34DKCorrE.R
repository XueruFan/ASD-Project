...
# 斯皮尔曼相关，Site作为控制变量，top 5个脑指标合成的新指标作为自变量,
...
plotDir <- file.path(abideDir, "Plot/Cluster/Cluster_A/SpectralCluster34DK/Corr/CorrE")
resDate <- "240315"
newDate <- "240610"
...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrE_L_", newDate, ".csv")
...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrE_H_", newDate, ".csv")
...
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrE_Final_", newDate, ".csv")
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("CorrE_", i, "_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
