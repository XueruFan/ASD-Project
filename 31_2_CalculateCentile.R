# this script is used to calculate ABIDE II centile
...
resultsDir <- file.path(abideDir, "Centile_2")
lifeSpanDir <- "E:/PhDproject/LBCC/lbcc"
resDate <- "240315"

abide_forCentile <- read.csv(file.path(dataDir, paste0("abide_2_forCentile_", resDate, ".csv")))
abide_analysis <- abide_forCentile[, -3:-2]
...
for (feature in volumeNames) {
  ...
  data <- read.csv(file.path(abideDir, "Centile_2", paste0(feature,".csv")))
...
# save result
name <- paste0("abide_2_centile_", resDate, ".csv")
...
name <- paste0("abide_2_centile_dev_", resDate, ".csv")
