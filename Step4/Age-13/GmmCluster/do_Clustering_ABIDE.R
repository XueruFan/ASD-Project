# Performe GMM clustering analysis using 34 regional volume OoS scores as classification
...
# Part 2: Use the optimal cluster number determined from last part to perform GMM clustering.
...
packages <- c("mclust", "cluster", "ggplot2", "Rtsne", "openxlsx")
...
resDir <- file.path(abideDir, "Analysis/Cluster/GMM513")
...
calculate_silhouette <- function(data, max_clusters = 10) {
  ...
  for (k in 2:max_clusters) {
    # 执行谱聚类
    set.seed(941205)
    gmm_model <- Mclust(data, G = k, verbose = FALSE)
...
################## Part 2: Use the optimal cluster number to perform GMM clustering ###########

# 执行GMM聚类
set.seed(941205)
gmm_model <- Mclust(data, G = optimal_clusters, verbose = FALSE)
