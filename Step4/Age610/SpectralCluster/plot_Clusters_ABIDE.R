...
resDir <- file.path(abideDir, "Analysis/Cluster/Spect610")
plotDir <- file.path(abideDir, "Plot/Cluster/Spect610")
resDate <- "240315"
newDate <- "240610"
...
ggseg(.data = asd_parc_centile, mapping = aes(fill = median), color = "black", atlas = dk,
  ...
  scale_fill_gradient(low = "#5378ac", high = "white", limits = c(0.13, 0.41),
                      breaks = seq(0.13, 0.41, by = 0.05)) +
...
ggseg(.data = abno_g1, mapping = aes(fill = perc), color = "black", atlas = dk,
  ...
  theme(legend.title = element_blank(), legend.position = "bottom",
        legend.key.width = unit(1, "cm")) +
  scale_fill_gradient(low = "white", high = "#d26b66", limits = c(0, 19),
