...
# 斯皮尔曼相关，Site和FIQ作为控制变量，34个脑指标分别作为自变量,
...
plotDir <- file.path(abideDir, "Plot/Cluster/Cluster_A/SpectralCluster34DK/Corr/CorrC")
resDate <- "240315"
newDate <- "240610"
...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrC_L_", newDate, ".csv")
...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrC_H_", newDate, ".csv")
...
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_34DK_statis_CorrC_Final_", newDate, ".csv")
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("CorrC_", i, "_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
