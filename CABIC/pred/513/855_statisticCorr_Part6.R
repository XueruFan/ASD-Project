...
for (name_brain in names_brain) {
  for (name_cog in names_cog_s) {
    ...
    if (sum(!is.na(y) & !is.na(x)) >= 30) {
      ...
      temp_L <- L[!is.na(y), c(name_brain, "SITE", "TCV", "Age", name_cog)]
      temp_L$y <- y[!is.na(y)]
      ...
      if (nrow(temp_L) >= 30) {
        if (length(unique(temp_L$SITE)) > 1) { # 确保 SITE 还有多个水平
          ...
          x_lm <- lm(as.formula(paste(name_brain, "~ SITE + TCV + Age")), data = temp_L)
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        ...
        if (nrow(temp_H) >= 30) {
          if (length(unique(temp_H$SITE)) > 1) { # 确保 SITE 还有多个水平
            ...
            x_lm <- lm(as.formula(paste(name_brain, "~ SITE + TCV + Age")), data = temp_H)
...
name <- paste0("corr_Part6_Select_H_", resDate, ".csv")
write.csv(H_sorted, file.path(resuDir, name), row.names = F)
name <- paste0("corr_Part6_Select_L_", resDate, ".csv")
...
name <- paste0("corr_Part6_Select_LH_", resDate, ".csv")
