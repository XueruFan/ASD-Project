...
names_brain <- c("precentral", "parahippocampal")

names_cog <- names(pheno)[9:ncol(pheno)]
# names_cog <- c("ADOS_SOCI", "ADOS_COMM", "ADOS_SA", "ADOS_RRB" ,"ADOS_TOTAL",
#                "RBS_STB", "RBS_SI", "RBS_CB", "RBS_RIB", "RBS_SAB", "RBS_REB", "RBS_TOTAL")
names_cog <- c("ADOS_SOCI", "RBS_SI")
...
name <- paste0("cabic_Part6_Select_H_", resDate, ".csv")
write.csv(H_sorted, file.path(resuDir, name), row.names = F)
### 保存下来结果
name <- paste0("cabic_Part6_Select_L_", resDate, ".csv")
