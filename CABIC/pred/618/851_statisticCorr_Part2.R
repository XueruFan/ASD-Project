...
resuDir <- file.path(cabicDir, "result/Corr")
plotDir <- file.path(resuDir, "Plot/Corr")
resDate <- "240928"
...
#### select: 控制变量、自变量、因变量
names_col <- c("predicted_cluster", "AGE", "SITE", names_brain, names_cog)
temp <- cabic_all[, names_col]
temp[temp < 0] <- NA
colnames(temp)[1] <- "clusterID"

temp618 <- subset(temp, AGE < 18 & AGE >= 6)

L <- subset(temp618, clusterID == "1")
L <- L[, -1]
H <- subset(temp618, clusterID == "2")
H <- H[, -1]
  ...
  for (name_cog in names_cog) {
    if (name_brain %in% names(L) & name_cog %in% names(L)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        # 创建临时数据框，包含做相关分析需要的列
        temp_L <- L[!is.na(y) & !is.na(x), c(name_brain, "SITE", name_cog)]
        temp_L$y <-  y[!is.na(y) & !is.na(x)]
        
        if (nrow(temp_L) >= 30) {
          if (length(unique(temp_L$SITE)) > 1) { # 确保 SITE 还有多个水平
            # 对 y 和 x 进行回归，控制 SITE
            y_lm <- lm(y ~ SITE, data = temp_L)
            x_lm <- lm(as.formula(paste(name_brain, "~ SITE")), data = temp_L)
...
for (name_brain in names_brain) {
  for (name_cog in names_cog) {
    if (name_brain %in% names(H) & name_cog %in% names(H)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        # 创建临时数据框，包含做相关分析需要的列
        temp_H <- H[!is.na(y) & !is.na(x), c(name_brain, "SITE", name_cog)]
        temp_H$y <-  y[!is.na(y) & !is.na(x)]
        
        if (nrow(temp_H) >= 30) {
          if (length(unique(temp_H$SITE)) > 1) { # 确保 SITE 还有多个水平
            # 对 y 和 x 进行回归，控制 SITE
            y_lm <- lm(y ~ SITE, data = temp_H)
            x_lm <- lm(as.formula(paste(name_brain, "~ SITE")), data = temp_H)
...
### 保存下来结果
name <- paste0("cabic_Part2_H_", resDate, ".csv")
write.csv(H_sorted, file.path(resuDir, name), row.names = F)
name <- paste0("cabic_Part2_L_", resDate, ".csv")
...
# }
# 
# name <- paste0("asd_male_dev_GC_corr_Part3_Site_LH_", newDate, ".csv")
...
#       scale_color_identity()
#   }
#   name <- paste0("GC_corr_Part3_Site_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
