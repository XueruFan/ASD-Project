...
#### select: 控制变量、自变量、因变量
names_col <- c("predicted_cluster", "SITE", "TCV", "Age", names_brain[-4], names_cog)
...
### 保存下来结果
name <- paste0("cabic_Part4_H_", resDate, ".csv")
write.csv(H_sorted, file.path(resuDir, name), row.names = F)
### 保存下来结果
name <- paste0("cabic_Part4_L_", resDate, ".csv")
write.csv(L_sorted, file.path(resuDir, name), row.names = F)

# Spearman 相关系数（cor_test$estimate）本身就可以作为一种效应量，因为它反映了两个变量之间的关联程度
# 0.1 - 0.3: 小效应
# 0.3 - 0.5: 中等效应
# > 0.5: 大效应
...
# name <- paste0("corr_Part4_MAIN_", newDate, ".csv")
