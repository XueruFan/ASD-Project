...
plotDir <- file.path(cabicDir, "result", "Plot", "Part3")
resDate <- "240928"
...
#### select: 控制变量、自变量、因变量
names_col <- c("predicted_cluster", "Age", names_brain, names_cog)
temp <- cabic_all[, names_col]
temp[temp < 0] <- NA
colnames(temp)[1] <- "clusterID"

temp <- subset(temp, Age < 18 & Age >= 6)
  ...
  for (name_cog in names_cog) {
    if (name_brain %in% names(L) & name_cog %in% names(L)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        ...
        temp_L <- L[!is.na(y) & !is.na(x) & !is.na(L$Age),
                    c(name_brain, "Age", name_cog)]
        temp_L$y <- y[!is.na(y) & !is.na(x) & !is.na(L$Age)]

        if (nrow(temp_L) >= 30) {
          # 对 y 和 x 进行回归，控制 SITE_ID 和 FIQ
          y_lm <- lm(y ~ Age, data = temp_L)
          x_lm <- lm(as.formula(paste(name_brain, "~ Age")), data = temp_L)
...
for (name_brain in names_brain) {
  for (name_cog in names_cog) {
    if (name_brain %in% names(H) & name_cog %in% names(H)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        # 创建临时数据框，包含做相关分析需要的列，并过滤掉 FIQ 为 NA 的行
        temp_H <- H[!is.na(y) & !is.na(x) & !is.na(H$Age),
                    c(name_brain, "Age", name_cog)]
        temp_H$y <- y[!is.na(y) & !is.na(x) & !is.na(H$Age)]

        if (nrow(temp_H) >= 30) {
            # 对 y 和 x 进行回归，控制 SITE_ID 和 FIQ
            y_lm <- lm(y ~ Age, data = temp_H)
            x_lm <- lm(as.formula(paste(name_brain, "~ Age")), data = temp_H)
...
name <- paste0("cabic_Part5_H_", resDate, ".csv")
write.csv(H_sorted, file.path(resuDir, name), row.names = F)
name <- paste0("cabic_Part5_L_", resDate, ".csv")
