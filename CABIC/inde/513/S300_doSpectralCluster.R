...
resuDir <- file.path(cabicDir, "result/inde/513")
plotDir <- file.path(resuDir, "Plot/Cluster")
newDate <- "240928"

cabic_centile <- read.csv(file.path(cabicDir, "result", paste0("cabic_centile_", newDate, ".csv")))

data_raw <- subset(cabic_centile, dx == "ASD" & sex == "Male")
data_raw <- subset(data_raw, Age < 13 & Age >= 5)
...
write.csv(plot_data, file.path(resuDir, name), row.names = F)
