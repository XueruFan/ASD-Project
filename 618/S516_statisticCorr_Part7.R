...
statiDir <- file.path(abideDir, "Analysis/Statistic/Spect618")
plotDir <- file.path(abideDir, "Plot/Cluster/Spect618/Corr")
resDate <- "240315"
newDate <- "240610"
...
#### select: 控制变量、自变量、因变量
names_col <- c("clusterID", "TCV_centile", "AGE_AT_SCAN", names_brain, names_cog)
temp <- All[, names_col]
temp[temp < 0] <- NA
...
### 保存下来结果
name <- paste0("corr_Part7_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)
### 保存下来结果
name <- paste0("corr_Part7_L_", newDate, ".csv")
...
name <- paste0("corr_Part7_MAIN_", newDate, ".csv")
write.csv(selected, file.path(statiDir, name), row.names = F)


#################### 筛选显著且相关大于0.2的结果

colnames(L_selected)[3:4] <- c("Lr", "Lp")
colnames(H_selected)[3:4] <- c("Hr", "Hp")
sorted <- full_join(L_selected[,1:4], H_selected[,1:4], by = c("name_cog", "name_brain"))
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("corr_Part7_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
