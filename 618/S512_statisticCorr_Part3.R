...
plotDir <- file.path(abideDir, "Plot/Cluster/Spect618/Corr/Part3")
resDate <- "240315"
newDate <- "240610"
...
#### select: 控制变量、自变量、因变量
names_col <- c("clusterID", "TCV_centile", names_brain, names_cog)
temp <- All[, names_col]
temp[temp < 0] <- NA
    ...
    if (name_brain %in% names(L) & name_cog %in% names(L)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        ...
        temp_L <- L[!is.na(y) & !is.na(x) & !is.na(L$TCV_centile),
                    c(name_brain, "TCV_centile", name_cog)]
...
### 保存下来结果
name <- paste0("corr_Part3_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)
### 保存下来结果
name <- paste0("corr_Part3_L_", newDate, ".csv")
...
name <- paste0("corr_Part3_MAIN_", newDate, ".csv")
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("corr_Part3_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
