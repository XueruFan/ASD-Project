...
#### select: 控制变量、自变量、因变量
names_col <- c("clusterID", "AGE_AT_SCAN", names_brain, names_cog)
temp <- All[, names_col]
temp[temp < 0] <- NA
  ...
  for (name_cog in names_cog) {
    if (name_brain %in% names(L) & name_cog %in% names(L)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        ...
        temp_L <- L[!is.na(y) & !is.na(x) & !is.na(L$AGE_AT_SCAN),
                    c(name_brain, "AGE_AT_SCAN", name_cog)]
...
for (name_brain in names_brain) {
  for (name_cog in names_cog) {
    if (name_brain %in% names(H) & name_cog %in% names(H)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        # 创建临时数据框，包含做相关分析需要的列，并过滤掉 FIQ 为 NA 的行
        temp_H <- H[!is.na(y) & !is.na(x) & !is.na(H$AGE_AT_SCAN),
                    c(name_brain, "AGE_AT_SCAN", name_cog)]
...
### 保存下来结果
name <- paste0("corr_Part5_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)
### 保存下来结果
name <- paste0("corr_Part5_L_", newDate, ".csv")
