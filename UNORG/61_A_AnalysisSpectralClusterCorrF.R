...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_statis_CorrF_L_", newDate, ".csv")
write.csv(L_sorted, file.path(statiDir, name), row.names = F)

################### H组

# 确保 SITE_ID 和 FIQ 是因子类型
...
### 保存下来结果
name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_statis_CorrF_H_", newDate, ".csv")
