...
plotDir <- file.path(abideDir, "Plot/Cluster/GMM513/GAMM")
...
for (volumeName in volumeNames) {
  ...
  for (cluster in unique(studyFIT$clusterID)) {
    ...
    # 准备预测数据（忽略站点）
    pred_data_no_site <- data.frame(Age = seq(5,13, length.out = 100))
  ...
  ggplot() +
    ...
    annotate("segment", x = 5, xend = 13, y = 0.5, yend = 0.5, color = "#e6e6e6",
