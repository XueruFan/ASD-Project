...
# Part 2: 34个脑区中贡献top5的指标合成的新指标作为自变量
# 保存原始的相关系数和p值csv文件，另外，筛选p小于0.01且r大于0.2的结果【无】
##################################
...
# 根据脑指标对于人群聚类的贡献进行加权
rank <- read.csv(file.path(clustDir, paste0("asd_male_Spectral_Cluster_RMrank_", newDate, ".csv")))
...
### 保存下来结果
name <- paste0("asd_male_dev_SC_corr_Part1_34_L_", newDate, ".csv")
...
### 保存下来结果
name <- paste0("asd_male_dev_SC_corr_Part1_34_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)




################### 筛选显著p < 0.01且相关大于0.2的结果
H_sorted <- H_sorted %>%
...
name <- paste0("asd_male_dev_SC_corr_Part1_34_LH_", newDate, ".csv")
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("SC_corr_Part1_34_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
...
### 保存下来结果
name <- paste0("asd_male_dev_SC_corr_Part1_05_L_", newDate, ".csv")
write.csv(L_sorted, file.path(statiDir, name), row.names = F)
...
### 保存下来结果
name <- paste0("asd_male_dev_SC_corr_Part1_05_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)
