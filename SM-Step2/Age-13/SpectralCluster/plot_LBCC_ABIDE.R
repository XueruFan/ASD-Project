...
plotDir <- file.path(abideDir, "Plot/Cluster/Spect513/LBCC")
newDate <- "240610"
...
for (volumeName in volumeNames){
  ...
  # 设置年龄区间
  ageTicks <- seq(5, 13, by = 2)  # 原始年龄间隔为2年
  ageLimits <- c(5, 13)  # 限制绘图的年龄范围为5到13岁
  ...
  ggplot(plotData_ABIDE, aes(x = (exp(AgeTransformed) - 280) / 365.25, y = y)) +
    ...
    # 修改x轴为原始年龄，间隔为2年
    scale_x_continuous(limits = c(5, 13), breaks = seq(5, 13, by = 2),
                       labels = c("5 yr", "7", "9", "11", "13")) +
    xlab("") + ylab("") +
