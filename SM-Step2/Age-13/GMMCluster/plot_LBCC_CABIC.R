...
resuDir <- file.path(cabicDir, "result/pred/GMM513")
plotDir <- file.path(cabicDir, "result/pred/GMM513/Plot/LBCC")
resDate <- "240928"
...
exampleFIT <- read.csv(file.path(resuDir, name))

volumeNames <- names(exampleFIT)[c(which(names(exampleFIT) == "paracentral"):
...
for (volumeName in volumeNames){
  ...
  ########## load abide data
  studyFIT <- read.csv(file.path(cabicDir, "result/Centile", paste0(volumeName,".csv")))
  studyFIT <- merge(exampleFIT, studyFIT, by = "participant", all.x = T)
  studyFIT1 <- subset(studyFIT, predicted_cluster == "1")
  studyFIT2 <- subset(studyFIT, predicted_cluster == "2")
