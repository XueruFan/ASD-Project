  ...
  for (name_cog in names_cog) {
    if (name_brain %in% names(L) & name_cog %in% names(L)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        ...
        temp_L <- L[!is.na(y) & !is.na(x) & !is.na(L$AGE_AT_SCAN), c(name_brain, "SITE_ID", "AGE_AT_SCAN", name_cog)]
        temp_L$y <- y[!is.na(y) & !is.na(x) & !is.na(L$AGE_AT_SCAN)]
        ...
        if (nrow(temp_L) >= 30) {
          if (length(unique(temp_L$SITE_ID)) > 1) { # 确保 SITE_ID 还有多个水平
            ...
            x_lm <- lm(as.formula(paste(name_brain, "~ SITE_ID + AGE_AT_SCAN")), data = temp_L)
            ...
            x_lm <- lm(as.formula(paste(name_brain, "~ AGE_AT_SCAN")), data = temp_L)
...
for (name_brain in names_brain) {
  for (name_cog in names_cog) {
    if (name_brain %in% names(H) & name_cog %in% names(H)) {
      ...
      if (sum(!is.na(y) & !is.na(x)) >= 30) {
        # 创建临时数据框，包含做相关分析需要的列，并过滤掉 FIQ 为 NA 的行
        temp_H <- H[!is.na(y) & !is.na(x) & !is.na(H$AGE_AT_SCAN), c(name_brain, "SITE_ID", "AGE_AT_SCAN", name_cog)]
        temp_H$y <- y[!is.na(y) & !is.na(x) & !is.na(H$AGE_AT_SCAN)]
        
        if (nrow(temp_H) >= 30) {
          if (length(unique(temp_H$AGE_AT_SCAN)) > 1) { # 确保 SITE_ID 还有多个水平
            # 对 y 和 x 进行回归，控制 SITE_ID 和 FIQ
            y_lm <- lm(y ~ SITE_ID + AGE_AT_SCAN, data = temp_H)
            x_lm <- lm(as.formula(paste(name_brain, "~ SITE_ID + AGE_AT_SCAN")), data = temp_H)
            ...
            x_lm <- lm(as.formula(paste(name_brain, "~ AGE_AT_SCAN")), data = temp_H)
...
name <- paste0("asd_male_dev_GC_corr_Part5_SiteAge_LH_", newDate, ".csv")
...
for (i in 1:nrow(sorted)) {
  ...
  name <- paste0("GC_corr_Part5_SiteAge_", to_plot_names[1], "_", to_plot_names[2], "_", newDate, ".png")
