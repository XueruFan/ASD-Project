...
statDir <- ("E:/PhDproject/CABIC/result/pred/510/StruCorr")
...
# 读取 Cluster 数据
cluster <- read.csv("E:/PhDproject/CABIC/result/pred/510/cabic_cluster_predictions_510_240928.csv")
colnames(cluster)[45] <- "clusterID"
cluster$clusterID[cluster$clusterID == "1"] <- "H"
cluster$clusterID[cluster$clusterID == "2"] <- "1"
cluster$clusterID[cluster$clusterID == "H"] <- "2"
