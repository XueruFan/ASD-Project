...
# Part 4: 画出两个分型34个脑区的异常（5%和95%）流行率，png和csv
# Part 5: 把两组人按照L组的异常率从大到小排列脑区，csv
###################################################
...
resDir <- file.path(abideDir, "Analysis/Cluster/SpectralCluster")
...
name <- paste0("asd_male_Spectral_Cluster_1_Centile_Brain_", newDate, ".png")
...
name <- paste0("asd_male_Spectral_Cluster_2_Centile_Brain_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 7.8, height = 3, units = "in", dpi = 500)
...
name <- paste0("asd_male_Spectral_Cluster_Centile_Global_", newDate, ".png")
...
all_data$Measure <- factor(all_data$Measure, 
                           levels = c("insula", "transversetemporal", "temporalpole", "frontalpole", 
                                      "supramarginal", "superiortemporal", "superiorparietal", "superiorfrontal", 
                                      "rostralmiddlefrontal", "rostralanteriorcingulate", "precuneus", "precentral", 
                                      "posteriorcingulate", "postcentral", "pericalcarine", "parstriangularis", 
                                      "parsorbitalis", "parsopercularis", "paracentral", "parahippocampal", 
                                      "middletemporal", "medialorbitofrontal", "lingual", "lateralorbitofrontal", 
                                      "lateraloccipital", "isthmuscingulate", "inferiortemporal", "inferiorparietal", 
                                      "fusiform", "entorhinal", "cuneus", "corpuscallosum", "caudalmiddlefrontal", 
                                      "caudalanteriorcingulate", "bankssts"),
                           labels = c("Insula", "Transverse Temporal", "Temporal Pole", "Frontal Pole", 
                                      "Supramarginal", "Superior Temporal", "Superior Parietal", "Superior Frontal", 
                                      "Rostral Middle Frontal", "Rostral Anterior Cingulate", "Precuneus", "Precentral", 
                                      "Posterior Cingulate", "Postcentral", "Pericalcarine", "Pars Triangularis", 
                                      "Pars Orbitalis", "Pars Opercularis", "Paracentral", "Parahippocampal", 
                                      "Middle Temporal", "Medial Orbitofrontal", "Lingual", "Lateral Orbitofrontal", 
                                      "Lateral Occipital", "Isthmus", "Inferior Temporal", "Inferior Parietal", 
                                      "Fusiform", "Entorhinal", "Cuneus", "Corpus Callosum", "Caudal Middle Frontal", 
                                      "Caudal Anterior Cingulate", "Banks of Superior Temporal Sulcus"))

ggplot(all_data, aes(x = Centile, y = Measure, fill = Measure, group = interaction(Group, Measure))) +
  geom_density_ridges(scale = 1.7, quantile_lines = TRUE, size = 1, quantiles = 2, alpha = .9) +
  scale_x_continuous(breaks = seq(0, 1, by = 0.25), labels = c("0%", "25%", "50%", "75%", "100%")) +
  ...
  theme(legend.position = "none", # without legend
        axis.text.y = element_text(size = 10, face = "bold"),
        axis.text.x = element_text(size = 10, face = "bold"))
name <- paste0("asd_male_Spectral_Cluster_Centile_Regional_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 5, height = 8, units = "in", dpi = 500)
...
name <- paste0("asd_male_Spectral_Cluster_1_Ab05rank_", newDate, ".csv")
...
ggseg(.data = abno_g1, mapping = aes(fill = perc), color = "black", atlas = dk,
  ...
  guides(fill = guide_colourbar(frame.colour = "black", frame.linewidth = 1, ticks = FALSE))
name <- paste0("asd_male_Spectral_Cluster_1_Ab05brain_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 7.8, height = 3, units = "in", dpi = 500)
# 大于95%
...
ggseg(.data = abno_g1, mapping = aes(fill = perc), color = "black", atlas = dk,
  ...
  scale_fill_viridis_c(option = "inferno") +
  guides(fill = guide_colourbar(frame.colour = "black", frame.linewidth = 1, ticks = FALSE))
name <- paste0("asd_male_Spectral_Cluster_1_Ab95brain_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 7.8, height = 3, units = "in", dpi = 500)
...
name <- paste0("asd_male_Spectral_Cluster_2_Ab95rank_", newDate, ".csv")
...
ggseg(.data = abno_g2, mapping = aes(fill = perc), color = "black", atlas = dk,
  ...
  scale_fill_viridis_c(option = "inferno") +
  guides(fill = guide_colourbar(frame.colour = "black", frame.linewidth = 1, ticks = FALSE))
name <- paste0("asd_male_Spectral_Cluster_2_Ab95brain_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 7.8, height = 3, units = "in", dpi = 500)
# 小于5%
...
ggseg(.data = abno_g2, mapping = aes(fill = perc), color = "black", atlas = dk,
  ...
  scale_fill_viridis_c(option = "inferno") +
  guides(fill = guide_colourbar(frame.colour = "black", frame.linewidth = 1, ticks = FALSE))
name <- paste0("asd_male_Spectral_Cluster_2_Ab05brain_", newDate, ".png")
ggsave(file.path(plotDir, name), width = 7.8, height = 3, units = "in", dpi = 500)
...
# 保存文件
name <- paste0("asd_male_Spectral_Cluster_AbPerc_", newDate, ".csv")
