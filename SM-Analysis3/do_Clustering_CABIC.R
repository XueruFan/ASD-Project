# Perform spectral clustering analysis using 34 regional volume OoS scores as classification
# features to subgroup male ASD individuals (<13 years) from the CABIC dataset.
# Xue-Ru Fan 2 May 2025 @BNU
###################################################
...
cabic_centile <- read.csv(file.path(cabicDir, paste0("result/cabic_centile_", oldDate, ".csv")))
...
################################# save plot
name <- paste0("Silhouette_", oldDate, ".png")
...
write.csv(data_cluster, file.path(resuDir, name), row.names = F)
