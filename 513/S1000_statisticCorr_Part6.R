...
plotDir <- file.path(abideDir, "Plot/Cluster/Spect513/Corr/Part6")
resDate <- "240315"
newDate <- "240610"
...
#### select: 控制变量、自变量、因变量
names_col <- c("clusterID", "SITE_ID", "TCV_centile", "AGE_AT_SCAN", names_brain, names_cog_p, names_cog_s)
...
### 保存下来结果
name <- paste0("corr_Part6_Select_H_", newDate, ".csv")
write.csv(H_sorted, file.path(statiDir, name), row.names = F)
### 保存下来结果
name <- paste0("corr_Part6_Select_L_", newDate, ".csv")
...
name <- paste0("corr_Part6_Select_LH_", newDate, ".csv")
